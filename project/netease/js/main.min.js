...
"use strict";function topNav(){function t(){i=document.querySelector(".h-logo"),getCookie().loginSuc&&1==getCookie().loginSuc?e():n()}function e(){a&&(a.parentNode.removeChild(a),a=void 0);var t='<button class="cursor-pointer h-checked"><i class="icon-check"></i><span>已关注</span><a class="black sub-font">取消</a></button>';d=html2node(t),i.appendChild(d),d.addEventListener("click",function(){removeCookie("loginSuc","/","localhost"),n()})}function n(){d&&(d.parentNode.removeChild(d),d=void 0);var t='<p style="display:inline-block;"><button class="h-check cursor-pointer"><i class="icon-plus"></i><span>关注</span></button><span class="sub-font">粉丝<span class="sub-font">45</span></span></p>';a=html2node(t),i.appendChild(a),a.firstChild.addEventListener("click",o)}function o(){addClass(document.body,"overflow-hidden");var t='\n\t\t<div class="l-bg animated">\n\t\t\t<div class="l-wrap">\n\t\t\t\t<i class="icon-close right cursor-pointer"></i>\n\t\t\t\t<form id="login-form">\n\t\t\t\t\t<legend>登录网易云课堂</legend>\n\t\t\t\t\t<p><input id="account" type="text" placeholder="账号" required></p>\n\t\t\t\t\t<p><input id="pwd" type="password" placeholder="密码" required></p>\n\t\t\t\t\t<p><button id="login-btn" >登录</button></p>\n\t\t\t\t</form>\n\t\t\t</div>\n\t\t</div>',n=html2node(t),o=n.querySelector(".l-wrap i"),i=n.querySelector("#login-form"),a=n.querySelector("#account"),d=n.querySelector("#pwd"),l=n.querySelector("#login-btn");document.body.appendChild(n),animateClass(n,"fadeIn"),o.addEventListener("click",function(){animateClass(n,"fadeOut",function(){n.parentNode.removeChild(n),delClass(document.body,"overflow-hidden")})}),i.addEventListener("submit",function(t){t.preventDefault();var o=new Ajax({data:{userName:md5(a.value),password:md5(d.value)},url:"http://study.163.com/webDev/login.htm",method:"get",async:!0,contentType:""});o.on("ok",function(){if(1==o.result){animateClass(n,"fadeOut",function(){n.parentNode.removeChild(n),delClass(document.body,"overflow-hidden")}),e();var t=new Date;t.setDate(t.getDate()+1),setCookie("loginSuc","1",t)}else d.value="",l.innerHTML="验证失败"}),o.on("notOk",function(){l.innerHTML="连接失败"}),o.get()})}var i,a,d;return{init:t}}
