...
"use strict";function mainCards(){function e(){l.addEventListener("click",t),r.addEventListener("click",s)}function t(e){if(e.preventDefault(),10==type){if("产品设计"==e.target.innerHTML)return;type=20,pageNo=1,addClass(e.target,"select"),delClass(e.target.previousElementSibling,"select"),a()}else{if("编程语言"==e.target.innerHTML)return;type=10,pageNo=1,addClass(e.target,"select"),delClass(e.target.nextElementSibling,"select"),a()}}function s(e){e.target.parentNode.querySelector(".select");if("LI"==e.target.tagName){var t=parseInt(e.target.innerHTML,10);if(pageNo==t)return;pageNo=t,a()}if("SPAN"==e.target.tagName)if("&lt;"==e.target.innerHTML){if(1==pageNo)return;pageNo--,a()}else if("&gt;"==e.target.innerHTML){if(8==pageNo)return;pageNo++,a()}}function n(){pageNo<=1?(delClass(o[0],"unselect"),delClass(o[1],"unselect"),addClass(o[0],"unselect")):pageNo>=8?(delClass(o[0],"unselect"),delClass(o[1],"unselect"),addClass(o[1],"unselect")):(delClass(o[0],"unselect"),delClass(o[1],"unselect"));for(var e=0;e<i.length;e++)delClass(i[e],"select"),pageNo==parseInt(i[e].innerHTML,10)&&(addClass(i[e],"select"),console.log(i[e],pageNo))}function a(){var e=new Ajax({data:{pageNo:pageNo,psize:psize,type:type},url:"http://study.163.com/webDev/couresByCategory.htm",method:"get",async:!0,contentType:""});e.on("ok",function(){var t=JSON.parse(e.result);c.innerHTML="";for(var s=0;s<t.list.length;s++){var n,a='\n\t<div class="m-card-wrap">\n\t\t<section class="cursor-pointer">\n\t\t\t<i class="img" style="background-image: url('+t.list[s].middlePhotoUrl+')"></i>\n\t\t\t<h4 class="title ellipsis">'+t.list[s].name+'</h4>\n\t\t\t<p class="num num-on-hover"><i class="icon-person"></i><span class="sub-font">'+t.list[s].learnerCount+' 人在学</span></p>\n\t\t\t<p class="provider prov-off-hover sub-font">'+t.list[s].provider+'</p>\n\t\t\t<p class="provider prov-on-hover sub-font">发布者: '+t.list[s].provider+"</p>",l='\t\t\n\t\t\t<p class="category sub-font">分类: '+t.list[s].categoryName+'</p>\n\t\t\t<p class="num num-off-hover"><i class="icon-person"></i><span class="sub-font">'+t.list[s].learnerCount+"</span></p>",r='\t\t\n\t\t\t<p class="category sub-font">分类: 全部课程</p>\n\t\t\t<p class="num num-off-hover"><i class="icon-person"></i><span class="sub-font">'+t.list[s].learnerCount+"</span></p>",i='\n\t\t\t<p class="price">¥ '+t.list[s].price.toFixed(2)+"</p>",o='\n\t\t\t<p class="price">免费</p>',p='\n\t\t\t\t<div class="description" title="'+t.list[s].description+'"><p>'+t.list[s].description+"</p></div>\n\t\t</section>\n\t</div>\n\t\t";n=t.list[s].categoryName?0!=t.list[s].price?a+l+i+p:a+l+o+p:0!=t.list[s].price?a+r+i+p:a+r+o+p;var u=html2node(n);c.appendChild(u)}}),e.on("notOk",function(){console.log(e.result)}),e.get(),n()}var l=document.querySelector(".m-tab"),r=document.querySelector(".m-page-nav"),i=r.querySelectorAll("li"),o=r.querySelectorAll("span"),c=document.querySelector(".m-cards");return{init:e,update:a}}
